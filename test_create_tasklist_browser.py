...
def test_browser_patterns():
    ...
    json_data = {
        ...
        "tasks": []  # Empty tasks array but still present
    }
    ...
    # Test 5: JSON with non-empty tasks array
    print("\n=== Test 5: JSON with Tasks Array ===")
    json_data = {
        "title": f"Tasks Array List {uuid.uuid4().hex[:8]}",
        "description": "With actual tasks array",
        "tasks": [
            {
                "title": "Task 1",
    ...
    print(f"Test 5 (JSON with Tasks Array): {'✓' if results[4] else '✗'}")
